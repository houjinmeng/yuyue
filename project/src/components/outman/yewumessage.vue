<template>
  <div>
    ...
    <!-- 底部按钮 -->
    <div class="btn">
      <div class="same" style="border-radius: 0.15rem 0 0 0.15rem;" @click="quxiao" v-show="status=='等待来访'">取消预约</div>
      <router-link to="/outman/person">
        <div class="same" style="border-radius: 0 0.15rem 0.15rem 0;" v-show="status=='等待来访'">返回</div>
      </router-link>
      <router-link to="/outman/person">
        <div class="same" style="border-radius:0.5rem;width:1.2rem" v-show="status!='等待来访'">返回</div>
      </router-link>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      status: window.sessionStorage.getItem("yewust"),
      url: "",
      data: {},
