<template>
  <div>
    <div style="font-size:0.24rem;text-align:center;padding:0.15rem;">{{status}}订单</div>
    <div class="card" style="height:5.5rem">
      <div class="card2" v-for="(item,index) in table" :key="index">
        ...
        <div>
          ...
          <!-- <span>{{item.time*1000|formatDateTwo}}</span> -->
        </div>
        <div>
...
export default {
  ...
  data() {
    return {
      status: window.sessionStorage.getItem("orderstatus"),
  ...
  methods: {
    getlist() {
      this.$http
        .post("/audit/search_order", this.$qs.stringify(this.data))
        .then(res => {
          ...
          if (this.status == "所有") {
            this.table = res.data;
          } else if (this.status == "未预约") {
            res.data.forEach(item => {
              if (item.status == "未预约" || item.status == "已拒绝,需重新预约") {
